...
- **The session cookie is `Secure` by default and must stay that way in production.** Browsers refuse to store a `Secure` cookie over plain `http://`, so local and LAN testing need `SYMPTOM_TRACKER_DEV_INSECURE_COOKIE=1`. It is opt-in rather than auto-detected so the deployed service cannot lose `Secure` by accident; the systemd unit does not set it. Do not "helpfully" infer it from the request scheme — a spoofable `X-Forwarded-Proto` would then be enough to downgrade the cookie.
